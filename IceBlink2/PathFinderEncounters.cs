...
namespace IceBlink2
{
    public class PathFinderEncounters
    {
        ...
        public List<Coordinate> findNewPoint(Creature crt, Coordinate end, bool ignorePcAndCreatures)
        {
            ...
            //also add code for blocking moving to close to map borders
            /*
                ...
                foreach (Creature cr in mod.currentEncounter.encounterCreatureList)
                {
                    ...
                    if ((cr != crt) && (cr.hp > 0) && grid[cr.combatLocX, cr.combatLocY] != 4)
                    {
                        ...
                        else if (crSize == 4)
                        {
                            ...
                            if ((cr.combatLocX < mod.currentEncounter.MapSizeX - 1) && (cr.combatLocY < mod.currentEncounter.MapSizeY - 1))
                            {
                                if (grid[cr.combatLocX + 1, cr.combatLocY + 1] != 4)
                                {
                                    grid[cr.combatLocX + 1, cr.combatLocY + 1] = 1;
        ...
        //Called from outside just to set up the pathNodes and return nothing
        public void setupPathNodes(Creature crt, Coordinate end, bool ignorePcAndCreatures)
        {
            findNewPoint(crt, end, ignorePcAndCreatures);
            //list.Clear();
        }
        ...
        public void resetGrid(Creature crt)
        {
            ...
            for (int col = 0; col < mod.currentEncounter.MapSizeY; col++)
            {
                for (int row = 0; row < mod.currentEncounter.MapSizeX; row++)
                {
                    ...
                    //block edges of map
                    int crtSize2 = crt.creatureSize;
                    //wide, block right edge
                    if (crtSize2 == 2)
                    {
                        if (row == mod.currentEncounter.MapSizeX - 1)
                        {
                            grid[row, col] = 1;
                        }
                    }
                    ...
                    if (crtSize2 == 3)
                    {
                        if (col == mod.currentEncounter.MapSizeY - 1)
                        {
                            grid[row, col] = 1;
                        }
                    }
                    //large, block right edge and bottom
                    if (crtSize2 == 4)
                    {
                        if (row == mod.currentEncounter.MapSizeX - 1)
                        {
        ...
        public void setupPathNodesCasterAI(Creature crt, Coordinate end, bool ignoreCrtsAndPCs)
        {
            pathNodes.Clear();
            foundEnd = false;
            Coordinate newPoint = new Coordinate(-1, -1);
        ...
        243 +                    }*/
            ...
            else
            {
                ...
                for (int i = 0; i < 9999; i++) //keep going until full path back to crt is built
                {
                    ...
                    if ((pathNodes[pathNodes.Count - 1].X == crt.combatLocX) && (pathNodes[pathNodes.Count - 1].Y == crt.combatLocY))
                    {
                        //remove the crt actual current location node from path
